 /*************************************************************************
 * 
 * Application file for 2048 Artificial Intelligence
 *
...
static float score_tilechoose_node(eval_state &state, board_t board, float cprob) {
	...
	if (cprob < CPROB_THRESH_BASE || state.curdepth >= CACHE_DEPTH_LIMIT) {
	...
	if (state.curdepth < CACHE_DEPTH_LIMIT) {
		trans_table_entry_t entry = { static_cast<__int8>(state.curdepth), res };
